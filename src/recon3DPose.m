...
%    - this way, we have the correct projection for the cylinders/sphere
    ...
    disp('Draw raw 3D model without body width alignment and texture mapping...');
    ...
    %alignment of each body part's width
    %cylindar's top mid and bot circle's raduis
    disp('Start to adjust each body part width...');
    ...
    Xnew1 = alignToCamera(pose.XnewR,camera.R,camera.t,R,t);
    figure(10);clf;
    disp('Draw 3D model with body width alignment but no texture mapping...');
    ...
    %start to find texture and make every texture normal face to audience
    shift_res = ones(1,9);
    shift_res_idx = 1;
    for i = all_idx
        kk = find(idx_xy == i);
        ...
        %four points decide the edge bounding box, but it need to make sure
        %these four points follow the same order
        if(i==2 ||i==5 ||i==8 ||i==11 ||i==13 ||i==15 ||i==18 ||i==20 ||i==23)
    ...
    [fid] = save3Dpose(pose.XnewR,pose.skel);
