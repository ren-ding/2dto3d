function [camera]= estimateWPCamera(xy,XY,param)
...
%param=0.8;
Standard_R = [1 0 0; 0 -1 0; 0 0 -1];
MM=Standard_R*param+R*(1-param);
%R=Standard_R;
[UU,SS,VV]=svd(MM);
R = UU*VV';
if det(R) < 0.9999999
    sv = diag(SS);
    [min_SV,min_idx_sv] = min(sv);
    u3 = UU(:,min_idx_sv);
    UU(:,min_idx_sv) = -u3; % www.ma.man.ac.uk/~nareports/narep161.pdf
end;
